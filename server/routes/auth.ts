    ...
    if (!user && !(await userRepository.count())) {
      ...
      if (
        ...
        if (user) {
          ...
          logger.info(
            'Sign-in attempt from Plex user with access to the media server; creating new SethFlix user',
            {
              label: 'API',
              ip: req.ip,
              email: account.email,
...
authRoutes.post('/jellyfin', async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      if (missingAdminUser) {
        logger.info(
          'Sign-in attempt from Jellyfin user with access to the media server; creating initial admin user for SethFlix',
          {
            label: 'API',
            ip: req.ip,
        ...
        logger.info(
          'Sign-in attempt from Jellyfin user with access to the media server; editing admin user for SethFlix',
          {
            label: 'API',
            ip: req.ip,
            jellyfinUsername: account.User.Name,
      ...
      const apiKey = await jellyfinClient.createApiToken('SethFlix');
    ...
    // User already exists, let's update their information
      ...
      logger.info(
        'Sign-in attempt from Jellyfin user with access to the media server; creating new SethFlix user',
        {
          label: 'API',
          ip: req.ip,
          jellyfinUsername: account.User.Name,
...
authRoutes.post('/local', async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !(await user.passwordMatch(body.password))) {
      logger.warn('Failed sign-in attempt using invalid SethFlix password', {
        label: 'API',
        ip: req.ip,
        email: body.email,
        userId: user?.id,
    ...
    logger.error('Something went wrong authenticating with SethFlix password', {
      label: 'API',
      errorMessage: e.message,
      ip: req.ip,
      email: body.email,
        ...
        try {
          ...
          try {
            await axios.delete(`${baseUrl}/Devices`, {
              ...
              headers: {
                'X-Emby-Authorization': `MediaBrowser Client="SethFlix", Device="SethFlix", DeviceId="seerr", Version="${getAppVersion()}", Token="${
